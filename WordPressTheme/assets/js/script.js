...
document.addEventListener("DOMContentLoaded", function () {
  var headerHeight = 0; // ヘッダーの高さを格納する変数
  var header = document.querySelector(".header"); // ヘッダー要素
  var anchorLinks = document.querySelectorAll('a[href*="#"]'); // ハッシュリンクを含むすべてのアンカーリンク
  ...
  var scrollToHash = function scrollToHash(hash) {
    ...
    var targetElement = document.querySelector(hash); // ハッシュに対応する要素を取得
    if (!targetElement) return; // 要素が存在しない場合は何もしない

    // ヘッダーの高さを再取得（最新の高さを反映）
    updateHeaderHeight();
    var scrollAction = function scrollAction() {
      var offset = targetElement.getBoundingClientRect().top + window.scrollY - headerHeight;
  ...
  anchorLinks.forEach(function (link) {
    link.addEventListener("click", function (e) {
      var href = link.getAttribute("href"); // リンクの href 属性を取得
      if (href.includes("#")) {
        var hash = href.split("#")[1] ? "#".concat(href.split("#")[1]) : null;

        // 現在のページURL（ハッシュを除く）
        var currentPageUrl = window.location.origin + window.location.pathname;
        // リンク先のURL（ハッシュを除く）
        var linkPageUrl = href.split("#")[0];
  ...
  window.addEventListener("load", function () {
    var hash = window.location.hash; // URLに含まれるハッシュを取得
    if (hash) {
