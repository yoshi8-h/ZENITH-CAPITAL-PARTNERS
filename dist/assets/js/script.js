...
document.addEventListener('DOMContentLoaded', function () {
  ...
  radioButtons.forEach(function (button) {
    radio = button.value;
    var id = button.closest('[id]').id;
    document.querySelector(".confirm_".concat(id)).textContent = radio;
  });

  // 入力フィールドの内容が変更された場合の処理
  var formInputs = document.querySelectorAll('.field input, .field select, .field textarea');
  formInputs.forEach(function (input) {
    input.addEventListener('change', function () {
      ...
      if (type === "radio") {
        ...
        check = this.value;
        var _id = this.closest("[id]").id;
        document.querySelector(".confirm_".concat(_id)).textContent += check + " / ";
      } else {
        var _id2 = this.id;
        document.querySelector(".confirm_".concat(_id2)).textContent = val;
      }
    });
  });
