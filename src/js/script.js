...
document.addEventListener("DOMContentLoaded", () => {
  let header = document.querySelector(".header"); // ヘッダー要素
  let headerHeight = header ? header.offsetHeight : 0; // ヘッダー高さを取得
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const emailInput = document.getElementById('your-email'); // メールアドレス
  const emailConfirmInput = document.getElementById('your-email-confirm'); // 確認用メールアドレス
  const textareaInput = document.getElementById('your-message'); // テキストエリア

  /**
   * 必須項目のエラーメッセージを表示する関数
   */
  const showError = (input, message) => {
    let errorTip = input.parentNode.querySelector('.wpcf7-not-valid-tip');
    if (!errorTip) {
   ...
   */
  const clearError = (input) => {
    const errorTip = input.parentNode.querySelector('.wpcf7-not-valid-tip');
    if (errorTip) {
...
document.addEventListener('DOMContentLoaded', function() {
  const fadeInUps = document.querySelectorAll(".js-fadeInUp-translate");  // ページ内の、このアニメーションをさせたい全ての要素を取得
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  cardContainers.forEach(container => {
    ...
    gsap.fromTo(cards, {y:35, autoAlpha:0}, {y:0, autoAlpha:1, stagger:.14, scrollTrigger:{
        trigger: container,
        start: 'top 70%',
        // markers:{
        //   startColor: "green",
